...
#         super().__init__()
#         self.row, self.col, self.row1, self.col1 = None, None, None, None
#         self.place_x, self.place_y = row, col
...
#                           height=95, width=95).place(x=x1, y=y1)
#         self.bind("<ButtonPress-1>", self.click1)
#         self.bind("<ButtonRelease-1>", self.click2)
#
#     def get_piece(self):
#         return field[self.row][self.col]
#
#     def click1(self, event):
#         self.row, self.col = event.widget.winfo_y() // 100, event.widget.winfo_x() // 100
#
#     def click2(self, event):
#         self.row1, self.col1 = event.widget.winfo_y() // 100, event.widget.winfo_x() // 100
...
'''def gathering_information(event):
    click_row, click_col = event.widget.winfo_y() // 100, event.widget.winfo_x() // 100
    piece = field[click_row][click_col]
    return click_row, click_col, piece'''
...
def button_pressed(event):
    ...
    if len(turns) == 2:
        ...
        if board.move_piece(row, col, row1, col1):
            ...
            if isinstance(piece, Pawn):
                img = white_pawn_pic if piece.get_color() == WHITE else black_pawn_pic
